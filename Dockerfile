...
RUN TAG=$(curl -s https://api.github.com/repos/databricks/cli/releases/latest | python3 -c "import sys,json; print(json.load(sys.stdin)['tag_name'])") && \
    VERSION=${TAG#v} && \
    curl -fsSL "https://github.com/databricks/cli/releases/download/${TAG}/databricks_cli_${VERSION}_linux_amd64.zip" -o /tmp/databricks.zip && \
